...
async fn durability_restart_recovers_via_wal() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db: DB<LocalFs, TokioExecutor> = builder.open_with_executor(Arc::clone(&exec)).await?;
    ...
    let reopened: DB<LocalFs, TokioExecutor> = builder.open_with_executor(exec).await?;
