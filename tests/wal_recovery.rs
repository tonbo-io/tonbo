...
async fn wal_recovers_rows_across_restart() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let extractor = projection_for_field(0, &DataType::Utf8)?;
    ...
    let extractor_recover = projection_for_field(0, &DataType::Utf8)?;
...
async fn wal_recovery_rewrite_after_truncated_tail() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let final_config =
        DynModeConfig::new(schema.clone(), projection_for_field(0, &DataType::Utf8)?)?;
...
async fn wal_recovery_survives_segment_rotations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let extractor_final = projection_for_field(0, &DataType::Utf8)?;
