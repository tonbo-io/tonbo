...
- Recovery behavior is controlled by `WalRecoveryMode`. For now, only `PointInTime`/`TolerateCorruptedTail` semantics are implemented; stricter `AbsoluteConsistency` and lenient `SkipCorrupted` modes remain TODO.
