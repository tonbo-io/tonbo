...
- `TxnAppend { provisional_id: u64, mode: Dyn, row_mode, payload }`: appends batches to the open txn.
  ...
  - Row modes (applies only to `mode=Dyn` for now):
    - `RowPayload (0)`: current behavior. Payload is the Arrow IPC batch containing all row columns plus appended `_commit_ts` (`UInt64`) and `_tombstone` (`Boolean`) sidecars.
...
- There are no value columns, no `_tombstone`, and no nullable key components (same constraints as upsert batches).
...
#### Recovery semantics
- `replay::scan(cfg.dir)` produces `(WalEvent, seq)` tuples ordered by `seq`, stopping at the first invalid frame. `RowPayload` frames decode to `WalEvent::DynAppend` (batch + `_tombstone` + `_commit_ts`), while `KeyDelete` frames decode to `WalEvent::DynDelete` that carries the delete schema batch.
