...
impl Clone for ValueRef<'_> {
    fn clone(&self) -> Self {
        match self {
            ...
            ValueRef::List(v) => ValueRef::List(v.clone()),
        }
    }
}
    ...
    pub fn from_array_ref(array: &'a ArrayRef, index: usize) -> Result<Self, ValueError> {
        ...
        match array.data_type() {
            ...
            DataType::List(_field) => {
                ...
                let _data = arr.value(index);

                // TODO: convert list array to value ref list
                todo!()
            }
...
impl ValueRef<'_> {
    ...
    pub fn to_owned(&self) -> Value {
        match self {
            ...
            ValueRef::List(v) => Value::List(v.iter().map(|v| v.to_owned()).collect()),
        }
    }
}
...
impl<'a> From<&'a Value> for ValueRef<'a> {
    fn from(value: &'a Value) -> Self {
        match value {
            ...
            Value::List(vec) => ValueRef::List(vec.iter().map(|v| v.into()).collect::<Vec<_>>()),
        }
    }
}
...
impl PartialEq for ValueRef<'_> {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (ValueRef::List(a), ValueRef::List(b)) => a.eq(b),
            _ => {
...
impl Ord for ValueRef<'_> {
    fn cmp(&self, other: &Self) -> Ordering {
        match (self, other) {
            ...
            (ValueRef::List(a), ValueRef::List(b)) => a.cmp(b),
            _ => {
                panic!("can not compare different types: {self:?} and {other:?}")
            }
        }
...
mod tests {
    ...
    fn test_list_value_ref_cmp() {
        {
            let l1 = ValueRef::List(vec![ValueRef::Int32(1), ValueRef::Int32(2)]);
            let l2 = ValueRef::List(vec![ValueRef::Int32(1), ValueRef::Int32(2)]);
            let l3 = ValueRef::List(vec![ValueRef::Int32(1), ValueRef::Int32(3)]);
            let l4 = ValueRef::List(vec![ValueRef::Int32(0), ValueRef::Int32(4)]);
            assert!(l1 == l2);
            assert!(l1 < l3);
            assert!(l3 > l4);
        }
        {
            let l1 = ValueRef::List(vec![
                ValueRef::List(vec![
                    ValueRef::Date32(1),
                    ValueRef::Date32(2),
                    ValueRef::Date32(3),
    ...
    fn test_list_value_ref_with_null_cmp() {
        {
            let l1 = ValueRef::List(vec![ValueRef::Date64(1), ValueRef::Null]);
            let l2 = ValueRef::List(vec![ValueRef::Date64(1), ValueRef::Null]);
            let l3 = ValueRef::List(vec![ValueRef::Null, ValueRef::Date64(3)]);
            let l4 = ValueRef::List(vec![ValueRef::Null, ValueRef::Null]);
            assert!(l1 == l2);
            assert!(l1 > l3);
            assert!(l4 < l3);
        }
        {
            ...
            let l2 = ValueRef::List(vec![ValueRef::Null]);
            let l3 = ValueRef::List(vec![ValueRef::List(vec![ValueRef::Null])]);
            let l4 = ValueRef::List(vec![ValueRef::List(vec![ValueRef::Null, ValueRef::Null])]);
            assert!(l1 < l2);
            assert!(l2 < l3);
            assert!(l3 < l4);
        }
        {
            let l1 = ValueRef::List(vec![ValueRef::List(vec![
                ValueRef::Float32(0.001),
                ValueRef::Float32(2.2),
                ValueRef::Float32(3.4),
            ])]);
            let l2 = ValueRef::List(vec![
                ValueRef::List(vec![
                    ValueRef::Null,
                    ValueRef::Float32(2222.2),
                    ValueRef::Float32(2223.4),
                ]),
                ValueRef::Null,
            ]);
            let l3 = ValueRef::List(vec![ValueRef::Null]);
            assert!(l1 > l2);
            assert!(l2 > l3);
        }
    }
